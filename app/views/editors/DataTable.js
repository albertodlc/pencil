var DataTable = function () {
    ...
    ActionColumn.prototype.getCellContentHtml = function (data, row, col) {
        ...
        for (var i = 0; i < this.actions.length; i ++) {
            ...
            html += "<span action-id=\"" + action.id + "\" class=\"mi " + action.type + " Action"  + action.id +  " " + (action.isApplicable && action.isApplicable(data) ? "" : "disabled")  + "\" title=\"" + NDom.htmlEncode(action.title) + "\"><i>" + action.icon + "</i></span>"
    ...
    DataTable.prototype.invalidateSelectionInfo = function(paginator) {
        ...
        paginator.getSelectedItems(function(items) {
            ...
            NDom.toggleClass(thiz.container, "HasSelectionInfo", items && items.length > 0);
        });
    }
